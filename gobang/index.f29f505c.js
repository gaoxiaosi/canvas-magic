...
close(){this.ctx.clearRect(0,0,this.W,this.H),this.canvas.onclick=null,document.getElementById(this.canvas.id).remove()}};let i=Math.min(window.innerWidth,window.innerHeight)/8,n=6*i,a="#E4A751",o=1,s="#000000",c=document.createElement("canvas"),r=c.getContext("2d");// 总步数
...
let d=Array.from({length:5},()=>[,,,,,].fill(-1)),f=!0,x=0,g=[];h.onclick=t=>{if(0===g.length)return;f=!f,x--;let{x:e,y:l}=g.pop();if(d[e][l]=-1,m(e,l),0===g.length)return;let{x:i,y:n}=g.at(-1);y(i,n);// let lastStep = steps.at(-1)
// drawRedPoint(lastStep.x, lastStep.y);
},// 监听棋盘点击位置
c.onclick=t=>{let[e,l]=[t.offsetX,t.offsetY].map(t=>Math.round(t/i)-1);if(d[e]?.[l]===-1){if(g.length>0){let{x:t,y:e,isBlack:l}=g.at(-1);m(t,e),w(t,e,l)}w(e,l,f),y(e,l),g.push({x:e,y:l,isBlack:f}),d[e][l]=f?1:2,b(e,l,d[e][l],d)?C(`${f?"黑":"白"}\u{68CB}\u{8D62}\u{4E86}!`):25==++x?C("游戏结束，平局！"):f=!f}};// 绘制棋盘（棋盘背景色 && 网格线）
const u=()=>{r.fillStyle=a,r.fillRect(0,0,n,n);for(let t=0;t<5;t++)p(0,t,4,t),p(t,0,t,4)},p=(t,e,l,n,a=o,c=s)=>{r.lineWidth=a,r.strokeStyle=c,r.beginPath(),r.moveTo(t*i+i,e*i+i),r.lineTo(l*i+i,n*i+i),r.stroke()},w=(t,e,l)=>{r.save(),r.shadowColor="rgba(0, 0, 0, 0.5)",r.shadowOffsetX=r.shadowOffsetY=.06*i,r.shadowBlur=.04*i,r.beginPath(),t=t*i+i,e=e*i+i,r.arc(t,e,.4*i,0,2*Math.PI),r.closePath();let n=r.createRadialGradient(t,e,0,t,e,.4*i);n.addColorStop(0,l?"#707070":"#FFFFFF"),n.addColorStop(1,l?"#000000":"#D5D8DC"),r.fillStyle=n,r.fill(),r.restore()},m=(t,e)=>{r.fillStyle=a,r.fillRect(t*i+i/2,e*i+i/2,i,i),p(t,e>0?e-.5:e,t,e<4?e+.5:e),p(t>0?t-.5:t,e,t<4?t+.5:t,e)},y=(t,e,l=.05*i)=>{r.beginPath(),r.fillStyle="#F56C6C",r.arc(t*i+i,e*i+i,l,0,2*Math.PI),r.closePath(),r.fill()},T=()=>{r.clearRect(0,0,n,n),u(),d=Array.from({length:5},()=>[,,,,,].fill(-1)),f=!0,x=0},C=t=>new l({canvas:c,onSuccess:T,primaryColor:a,textTitle:t}),b=(t,e,l,i)=>{for(let[n,a]of[[1,0],[0,1],[1,1],[1,-1]]){let[o,s,c]=[1,0,0];// count：连续同种颜色棋子数
// 判断：1、坐标是否越界 2、该位置的值是否一致（都是黑或白） 3、累计的次数是否达到5
for(;o<5&&i[t+n*++s]?.[e+a*s]===l;)o++;for(;o<5&&i[t-n*++c]?.[e-a*c]===l;)o++;if(5===o)return p(t+n*(s=4-c),e+a*s,t-n*c,e-a*c,5,"#F05459"),!0}return!1};window.onresize=(e=()=>{if(console.log("onresize"),r.clearRect(0,0,n,n),i=Math.min(window.innerWidth,window.innerHeight)/8,n=6*i,c.width=c.height=n,c.style.cssText=`display: block;margin: ${(window.innerHeight-n)/2}px auto;box-shadow: 0 0 5px #dcdcdc;cursor: pointer;`,u(),g.forEach(({x:t,y:e,isBlack:l})=>{w(t,e,l)}),g.length>0){let{x:t,y:e}=g.at(-1);y(t,e)}},t=null,(...l)=>{clearTimeout(t),t=setTimeout(()=>{e(...l)},512)}),window.onload=u;