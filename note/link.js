...
canvas.width = Math.floor(SIDE_W * DPR);
canvas.height = Math.floor(SIDE_H * DPR);
...
const drawTitle = () => {
  ...
  ctx.fillText(BG_TITLE, SIDE_W / 2, SIDE_H / 2);
}
...
const drawBlock = (x, y, valIndex, w = W) => {
  ...
  //  如果背景有字之类的要显示的东西，可以给图片加上一个和背景颜色一样的边框
  ctx.beginPath();
  ctx.strokeStyle = BOARD_COLOR;
  ctx.lineWidth = SPACE;
  ctx.strokeRect(x - SPACE / 2, y - SPACE / 2, w + SPACE, w + SPACE)
  ctx.stroke()
}
...
const drawLine = (points = [p1, p2, p3, p4], lineWidth = LINE_WIDTH, lineColor = LINE_COLOR) => {
  ...
  if (points[len - 1].x === points[len - 2].x) points[len - 1].oy = +(points[len - 1].y < points[len - 2].y)
  if (points[len - 1].y === points[len - 2].y) points[len - 1].ox = +(points[len - 1].x < points[len - 2].x)
