...
const autoPlay = async () => {
  isAutoPlay = true;
  let { x: x1, y: y1 } = tipsPoints[0], { x: x4, y: y4 } = tipsPoints[tipsPoints.length - 1]
  addFocus(x1, y1);
  addFocus(x4, y4);
  drawLine(tipsPoints);
  data[x1][y1] = -1;
...
/**
 * 检测两点是否可连接（返回true或false）
 * @param {Object} p1 { x: -1, y: -1 }
 * @param {Object} p4 { x: -1, y: -1 }
 * @returns false or [点位]，PS：返回的数据类型有两种不太好，但为了判断方便，就这样吧，一般函数不建议这样写
 */
const check = (p1, p4) => {
  ...
  for (let list of Object.values(obj)) {
    ...
    for (let i = 0; i < list.length; i++) {
      for (let j = i + 1; j < list.length; j++) {
        if (tipsPoints = check(list[i], list[j])) return true
      }
    }
  }
  return false;
...
const drawLine = (points, lineWidth = LINE_WIDTH, lineColor = LINE_COLOR) => {
...
const paint = () => {
  ...
  if (!isWin()) {
    ...
    timer = setTimeout(() => {
      let { x: x1, y: y1 } = tipsPoints[0],
          { x: x4, y: y4 } = tipsPoints[tipsPoints.length - 1];
      addFocus(x1, y1, 'red');
      addFocus(x4, y4, 'red');
    }, SHOW_TIPS_DURATION);
  }
