 ...
 */class{/**
   ...
   */constructor({canvas:t,onSuccess:e,onCancel:l,primaryColor:i="#F56C6C",maskColor:n="rgba(128, 128, 128, 0.6)",btnTextColor:a="#FFFFFF",textTitle:o="游戏结束",successBtnText:s="重新开始",cancelBtnText:r="取消"}){Object.assign(this,{onSuccess:e,onCancel:l,primaryColor:i,maskColor:n,btnTextColor:a,textTitle:o,successBtnText:s,cancelBtnText:r,W:t.width,H:t.height,titleH:50,titleBtnSpacing:50,btnH:40,btnPadding:10,btnSpacing:40// 按钮间距
...
drawButtons(){let{canvas:t,ctx:e,H:l,W:i,btnTextColor:n,primaryColor:a,onSuccess:o,onCancel:s,titleH:r,titleBtnSpacing:c,successBtnText:d,cancelBtnText:h,btnH:f,btnPadding:x,btnSpacing:u}=this;// 设置按钮样式（需提前设置字体大小，这样通过measureText方法计算文本内容的宽度）
e.font=`${f-2*x}px Arial`,e.textBaseline="top",e.textAlign="left";let p=e.measureText(d).width+2*x,g=e.measureText(h).width+2*x,m=(l+r+c)/2-f/2,w=(i-p-g-u)/2,y=w+p+u,T=[w,w+p,m,m+f],C=[y,y+g,m,m+f];// 取消按钮所占矩形范围（左右X的范围，上下Y的范围）
// 绘制成功和取消按钮
e.fillStyle=a,e.fillRect(w,m,p,f),e.fillRect(y,m,g,f),e.fillStyle=n,e.fillText(d,w+x,m+x),e.fillText(h,y+x,m+x);// 判断坐标是否在矩形（按钮）范围之内
...
close(){this.ctx.clearRect(0,0,this.W,this.H),this.canvas.onclick=null,document.getElementById(this.canvas.id).remove()}}});var o=a("8PRdk");const s="#E4A751",r=1,c="#000000";// 总步数
let d=Math.min(window.innerWidth,window.innerHeight)/18,h=16*d,f=document.createElement("canvas"),x=f.getContext("2d");// 边长 = 棋盘宽高
f.width=f.height=h,f.style.cssText="position: absolute; inset: 0; margin: auto; box-shadow: 5px 5px 20px #888888; cursor: pointer;",document.body.appendChild(f);// document.body.style.backgroundColor = '#f5f3e1'
let u=document.createElement("button");u.innerText="悔棋",u.style.cssText="display: block; position: absolute; left: 10px; top: 10px; padding: 15px 30px; font-size: 20px; cursor: pointer;",document.body.appendChild(u);// 记录棋盘的黑白棋，15*15的二维数组，初始值：0，黑棋：1，白棋：2
