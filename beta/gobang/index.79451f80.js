...
close(){this.ctx.clearRect(0,0,this.W,this.H),this.canvas.onclick=null,document.getElementById(this.canvas.id).remove()}}});var o=a("8PRdk");const r="#E4A751",s=1,c="#000000";// 总步数
let h=Math.min(window.innerWidth,window.innerHeight)/18,d=16*h,f=document.createElement("canvas"),u=f.getContext("2d");// 边长 = 棋盘宽高
f.width=f.height=d,f.style.cssText="position: absolute; inset: 0; margin: auto; box-shadow: 5px 5px 20px #888888; cursor: pointer;",document.body.appendChild(f);let x=document.createElement("button");x.innerText="悔棋";// regretBtn.style.cssText = 'display: block; position: absolute; left: 10px; top: 10px; padding: 15px 30px; font-size: 20px; cursor: pointer;'
...
f.onclick=t=>{let[e,l]=[t.offsetX,t.offsetY].map(t=>Math.round(t/h)-1);if(p[e]?.[l]===-1){if(w.length>0){let{x:t,y:e,isBlack:l}=w.at(-1);b(t,e),C(t,e,l)}C(e,l,g),v(e,l),w.push({x:e,y:l,isBlack:g}),p[e][l]=g?1:2,k(e,l,p[e][l],p)?B(`${g?"黑":"白"}\u{68CB}\u{8D62}\u{4E86}!`):225==++m?B("游戏结束，平局！"):g=!g}};// 绘制棋盘（棋盘背景色 && 网格线）
const T=()=>{u.fillStyle=r,u.fillRect(0,0,d,d);for(let t=0;t<15;t++)y(0,t,14,t),y(t,0,t,14)},y=(t,e,l,i,n=s,a=c)=>{u.lineWidth=n,u.strokeStyle=a,u.beginPath(),u.moveTo(t*h+h,e*h+h),u.lineTo(l*h+h,i*h+h),u.stroke()},C=(t,e,l)=>{u.save(),u.beginPath(),t=t*h+h,e=e*h+h,u.arc(t,e,.4*h,0,2*Math.PI),u.closePath(),u.shadowColor="rgba(0, 0, 0, 0.5)",u.shadowOffsetX=u.shadowOffsetY=.06*h,u.shadowBlur=.04*h;let i=u.createRadialGradient(t,e,0,t,e,.4*h);i.addColorStop(0,l?"#707070":"#FFFFFF"),i.addColorStop(1,l?"#000000":"#D5D8DC"),u.fillStyle=i,u.fill(),u.restore()},b=(t,e)=>{u.fillStyle=r,u.fillRect(t*h+h/2,e*h+h/2,h,h),y(t,e>0?e-.5:e,t,e<14?e+.5:e),y(t>0?t-.5:t,e,t<14?t+.5:t,e)},v=(t,e,l=.05*h)=>{u.beginPath(),u.fillStyle="#F56C6C",u.arc(t*h+h,e*h+h,l,0,2*Math.PI),u.closePath(),u.fill()},S=()=>{u.clearRect(0,0,d,d),T(),p=Array.from({length:15},()=>Array(15).fill(-1)),g=!0,m=0,w=[]},B=t=>new o.default({canvas:f,onSuccess:S,primaryColor:r,textTitle:t}),k=(t,e,l,i)=>{for(let[n,a]of[[1,0],[0,1],[1,1],[1,-1]]){let[o,r,s]=[1,0,0];// count：连续同种颜色棋子数
// 判断：1、坐标是否越界 2、该位置的值是否一致（都是黑或白） 3、累计的次数是否达到5
for(;o<5&&i[t+n*++r]?.[e+a*r]===l;)o++;for(;o<5&&i[t-n*++s]?.[e-a*s]===l;)o++;if(5===o)return y(t+n*(r=4-s),e+a*r,t-n*s,e-a*s,5,"#F05459"),!0}return!1};window.onresize=(e=()=>{if(u.clearRect(0,0,d,d),h=Math.min(window.innerWidth,window.innerHeight)/18,d=16*h,f.width=f.height=d,T(),w.forEach(({x:t,y:e,isBlack:l})=>{C(t,e,l)}),w.length>0){let{x:t,y:e}=w.at(-1);v(t,e)}},t=null,(...l)=>{clearTimeout(t),t=setTimeout(()=>{e(...l)},512)}),window.onload=T;