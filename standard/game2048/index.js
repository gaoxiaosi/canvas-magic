...
const animate = (animationGroup, diretion, duration = DURATION) => new Promise(resolve => {
  const linear = (groups, ratio) => groups.map(({ index, val, start, end }) => {
    ...
    if (diretion === 'ArrowRight') return { x: start + delta, y: index, val }
  })

  let start = null, elapsed = null, activeGroup = [], animateId = null;
  const draw = timestamp => {
    ...
    if (elapsedTime < duration) { // 判断是否绘制完成
      activeGroup = linear(animationGroup, elapsed / duration);
