...
e.font=`${h-2*x}px Arial`,e.textBaseline="top",e.textAlign="left";let p=e.measureText(d).width+2*x,g=e.measureText(f).width+2*x,m=(l+s+c)/2-h/2,C=(n-p-g-u)/2,T=C+p+u,w=[C,m,C+p,m+h],v=[T,m,T+g,m+h];// 取消按钮所占矩形范围（左上XY，右下XY）
// 绘制成功和取消按钮
...
t.onclick=t=>[o,r][[w,v].findIndex(e=>b(t.offsetX,t.offsetY,e)&&(this.close(),!0))]?.()}// 关闭模态窗（移除DOM节点）
