...
drawButtons(){let{canvas:t,ctx:e,H:l,W:n,btnTextColor:i,primaryColor:a,onSuccess:o,onCancel:r,titleH:s,titleBtnSpacing:c,successBtnText:d,cancelBtnText:f,btnH:h,btnPadding:u,btnSpacing:x}=this;// 设置按钮样式（需提前设置字体大小，这样通过measureText方法计算文本内容的宽度）
e.font=`${h-2*u}px Arial`,e.textBaseline="top",e.textAlign="left";let p=e.measureText(d).width+2*u,g=e.measureText(f).width+2*u,m=(l+s+c)/2-h/2,C=(n-p-g-x)/2,T=C+p+x,v=[C,m,C+p,m+h],w=[T,m,T+g,m+h];// 取消按钮所占矩形范围（左上XY，右下XY）
// 绘制成功和取消按钮
e.fillStyle=a,e.fillRect(C,m,p,h),e.fillRect(T,m,g,h),e.fillStyle=i,e.fillText(d,C+u,m+u),e.fillText(f,T+u,m+u);// 判断坐标是否在矩形（按钮）范围之内
let b=(t,e,[l,n,i,a])=>t>=l&&t<=i&&e>=n&&e<=a;// 监听点击事件，若点击到成功按钮，执行成功回调；若点击到取消按钮，执行取消回调。（只要点击到按钮都会关闭模态窗）
...
d.width=d.height=o,d.style.cssText="display: block; inset: 0; margin: auto;cursor: pointer;",document.body.appendChild(d);// 记录棋盘的黑白棋，15*15的二维数组，初始值：0，黑棋：1，白棋：2
let h=Array.from({length:15},()=>Array(15).fill(-1)),u=!0,x=0;// 下棋步数
// 监听棋盘点击位置
d.onclick=t=>{let[e,l]=[t.offsetX,t.offsetY].map(t=>Math.round(t/a)-1);h[e]?.[l]===-1&&(m(e,l,u),h[e][l]=u?1:2,v(e,l,h[e][l],h)?T(`${u?"黑":"白"}\u{68CB}\u{8D62}\u{4E86}!`):225==++x?T("游戏结束，平局！"):u=!u)};// 绘制棋盘（棋盘背景色 && 网格线）
const p=()=>{f.fillStyle=r,f.fillRect(0,0,o,o);for(let t=0;t<15;t++)g(0,t,14,t),g(t,0,t,14)},g=(t,e,l,n,i=s,o=c)=>{f.lineWidth=i,f.strokeStyle=o,f.beginPath(),f.moveTo(t*a+a,e*a+a),f.lineTo(l*a+a,n*a+a),f.stroke()},m=(t,e,l)=>{f.save(),f.beginPath(),t=t*a+a,e=e*a+a,f.arc(t,e,.4*a,0,2*Math.PI),f.closePath(),f.shadowColor="rgba(0, 0, 0, 0.5)",f.shadowOffsetX=f.shadowOffsetY=.06*a,f.shadowBlur=.04*a;let n=f.createRadialGradient(t,e,0,t,e,.4*a);n.addColorStop(0,l?"#707070":"#FFFFFF"),n.addColorStop(1,l?"#000000":"#D5D8DC"),f.fillStyle=n,f.fill(),f.restore()},C=()=>{f.clearRect(0,0,o,o),p(),h=Array.from({length:15},()=>Array(15).fill(-1)),u=!0,x=0},T=t=>new i.default({canvas:d,onSuccess:C,primaryColor:r,textTitle:t}),v=(t,e,l,n)=>{for(let[i,a]of[[1,0],[0,1],[1,1],[1,-1]]){let[o,r,s]=[1,0,0];// count：连续同种颜色棋子数
