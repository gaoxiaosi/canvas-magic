...
canvas.style.cssText = `display: block; margin: ${(window.innerHeight - SL) / 2}px auto;cursor: pointer;`;
...
const handleResize = () => {
  ...
  SL = W * (SIZE + 1);
  canvas.width = canvas.height = SL; // 棋盘宽高 = 边长
  canvas.style.cssText = `display: block;margin: ${(window.innerHeight - SL) / 2}px auto;box-shadow: 0 0 5px #dcdcdc;cursor: pointer;`;
  drawBoard();
